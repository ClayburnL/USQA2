...
% main_path='R:\Ultrasound\_STH\JWH\Scan Dept\_Monthly\';
cd(main_path);
folders=dir('*.*');

for i=3:3 %(length(folders)-2)
        path=strcat(main_path,folders(i).name);
        path=strcat(path,'\');
        cd(path);
        subfolders=dir('*.*');
%         if i==(length(folders)-2) % catch exception folder with an extra file for some reason
        if i==7
